...
int main(void)
{
    ...
    // Checking if the list is null as if the list->next is null on any node
    // it means that list have no more elements.
    while (list != NULL)
    {                   
        node *temp = list->next;  // getting the address of next node.
        free(list);    // free one node from which the address of next node is taken into temp.
        list = temp;   // copying address of node next to the node which was just freed.
    }
}